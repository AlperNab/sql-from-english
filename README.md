...
**Domain:** `Data / Analytics` · **Suite:** `Developer Productivity Suite` · **Accent:** `#60a5fa`
...
**Sql From English** is a standalone, browser-based AI workflow app for **Data / Analytics**. It turns structured inputs, uploaded files, and project-specific settings into reviewable outputs using a deterministic local engine plus optional local/cloud LLM enhancement.
...
|---|---:|:---:|---|---|
| `natural_language`<br>Natural language | select | Yes | English | Affects input: Natural language. |
...
| `dialect`<br>dialect | text | No | — | Affects customization: dialect. |
| `read_only_mode`<br>read-only mode | select | No | — | Affects customization: read-only mode. |
| `table_allowlist`<br>table allowlist | textarea | No | — | Affects customization: table allowlist. |
| `max_rows`<br>max rows | text | No | — | Affects customization: max rows. |
| `cost_guardrails`<br>cost guardrails | text | No | — | Affects customization: cost guardrails. |
| `join_policy`<br>join policy | text | No | — | Affects customization: join policy. |
| `explanation_level`<br>explanation level | select | No | — | Affects customization: explanation level. |
...
uvicorn app.main:app --host 127.0.0.1 --port 9155
```
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9155`. |
...
```bash
curl http://127.0.0.1:9155/api/health
```
...
```bash
curl -X POST http://127.0.0.1:9155/api/run \
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9155 uvicorn app.main:app --host 0.0.0.0 --port 9155
```
...
- `plugins/sql-from-english.json` for plugin metadata.
